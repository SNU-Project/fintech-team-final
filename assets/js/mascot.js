/* ============================================================
   마스코트 캐릭터 (v22 "닥터" 컨셉) — 청진기를 목에 건 간단한
   벡터(SVG) 캐릭터. 실제 일러스트가 준비되기 전까지 쓰는 자리표시자
   라서, 포즈 이름(greet/good/reassure)만으로 호출하고 나중에 이
   파일 하나만 실제 일러스트로 바꿔 끼우면 되게 컴포넌트를 분리했다
   (호출부는 Mascot.svg(pose)만 알면 된다). 외부 라이브러리·이미지
   파일 없이 인라인 SVG라 다양한 크기에서도 안 깨진다.
   ============================================================ */
(function (global) {
  "use strict";
  ...
  function svg(pose = "greet", opts = {}) {
    ...
    return `
      <svg class="mascot-svg ${className}" width="${size}" height="${size}" viewBox="0 0 100 100" role="img" aria-label="연봉닥터 캐릭터" style="overflow:visible">
        ...
        ${face.eyes}
        ${face.mouth}
        <!-- 청진기: 목에 걸친 튜브 + 양쪽 이어피스 + 아래로 늘어진 청진판 -->
        <path d="M20 34q0 20 14 22M80 34q0 20-14 22" stroke="#fff" stroke-width="3.2" fill="none" stroke-linecap="round"/>
        <circle cx="20" cy="32" r="3" fill="#fff"/>
        <circle cx="80" cy="32" r="3" fill="#fff"/>
        <path d="M50 56v14a8 8 0 0 0 8 8" stroke="#fff" stroke-width="3.2" fill="none" stroke-linecap="round"/>
        <circle cx="60" cy="78" r="5.5" fill="none" stroke="#fff" stroke-width="3"/>
      </svg>`;
  }
