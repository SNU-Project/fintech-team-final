...
**현재 화면에 가정값은 하나도 없습니다.** 예금도 OECD 한국 3개월 은행간금리(IR3TIB)를 월 복리로 굴린 실데이터입니다. 가정값을 새로 도입해야 한다면 반드시 화면과 `meta.json`의 `assumptions`에 함께 노출하세요. (아이스 아메리카노 지수는 체감 비교용 가정값 예외로 한동안 화면에 있었으나, v13에서 카드 3 개편과 함께 제거했습니다 — 관련 계산·상수도 함께 정리했습니다.)
...
**흐름**: 홈에서 설문 → 연봉닥터 진단 카드뉴스(처방전으로 마무리) → 아래 4개 탭의 상세 리포트
